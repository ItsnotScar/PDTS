...
try{
  ...
  $stmt = $conn->prepare("DELETE FROM complaints WHERE is_deleted=1 AND id IN ($place)");
  $stmt->bind_param($types, ...$ids);
  $stmt->execute(); $aff=$stmt->affected_rows; $stmt->close();
  $_SESSION['success_message'] = "Permanently removed $aff ticket(s).";
}catch(Throwable $e){ $_SESSION['error_message']='Purge failed: '.$e->getMessage(); }
